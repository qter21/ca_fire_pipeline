"""Complete CCP code test - All stages including multi-version."""
...
def main():
    ...
    print('='*80)
    print(f'COMPLETE CCP CODE TEST - ALL STAGES')
    print('='*80)
    print()
    ...
    def progress_callback(processed, total):
        ...
        if (now - last_update).total_seconds() >= 30 or processed == total:
            ...
            print(f'   [{datetime.now().strftime("%H:%M:%S")}] {processed}/{total} ({pct:.1f}%) | Rate: {rate:.2f}/s | ETA: {eta/60:.1f} min')
            last_update = now
    ...
    if result2["multi_version_count"] > 0:
        ...
        if result3["failed_sections"]:
            print(f'   Failed: {result3["failed_sections"]}')
        print()
    else:
    ...
    print(f'   With version data: {with_versions}')
    print(f'   Overall success: {with_content + with_versions}/{total_in_db} ({(with_content + with_versions)/total_in_db*100:.2f}%)')
    print()
    ...
    print('='*80)
    print('🎉 COMPLETE CCP TEST SUMMARY')
    print('='*80)
    print()
    print(f'Code: {code}')
